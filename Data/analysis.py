...
append_model_statistic_to_dfs(bertmodel, 'Average Word Similarity')
...
# STRIPPED DATA TO THOSE WITH QUESTION MARKS/NO BLANKS
for i in range(7):
    df = pandas.read_pickle(f'weekday{i}stripped.pkl')
    df = df[df['Clue'].str.contains("?", regex=False)]
    print(df)
    # df.to_pickle(f'weekday{i}stripped.pkl')
...
print(summary)
summary.to_pickle('questiondatanoblanks.pkl')


# for i in range(7):
#     dfq = pandas.read_pickle(f'weekday{i}question.pkl')
#     dfall = pandas.read_pickle(f'weekday{i}stripped.pkl')
#     data.append([weekdays[i], len(dfq.index)/len(dfall.index)])
    
# summary = pandas.DataFrame(data, columns=['Weekday', 'Proportion of Clues Containing [?]'])
# print(summary)
# summary.to_pickle('summarynoblanks.pkl')